...
public class BajaPelicula implements WindowListener, ActionListener {
	Frame ventana = new Frame("Baja Pelicula");
	Dialog dlgConfirmacion = new Dialog(ventana, "Segur@?", true);
	Dialog dlgMensaje = new Dialog(ventana, "Mensaje", true);

	Label lblElegir = new Label("Elegir el Pelicula a borrar:");
	Label lblConfirmar = new Label("Seguro de borrar la Pelicula ");
	Label lblMensaje = new Label("Eliminacion Correcta");

	Choice choPelicula = new Choice();
	...
	BajaPelicula() {
		...
		conexion.rellenarChoicePelicula(choPelicula);
		ventana.add(choPelicula);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(btnCancelar)) {
			...
			if (choPelicula.getSelectedIndex() != 0) {
				dlgConfirmacion.setLayout(new FlowLayout());
				dlgConfirmacion.setSize(270, 100);
				dlgConfirmacion.addWindowListener(this);

				lblConfirmar.setText("Seguro de borrar la Pelicula '" + choPelicula.getSelectedItem() + "'?");
			...
			String tabla[] = choPelicula.getSelectedItem().split("-");
			int resultado = conexion.eliminarPelicula(tabla[0]); // idUsuario --> DELETE
